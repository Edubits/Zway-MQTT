...
MQTTClient.prototype._onTimeout = function () {
  ...
  // Ensure graceful shutdown of socket if still open, else notify of disconnect
  if (this._connection != null)
    this._connection.close();
  else if (typeof this._disconnectCallback == 'function')
    this._disconnectCallback();
};
...
MQTTClient.messageHandlers[MQTTClient.messageTypes.PUBREL] = function (self, fixed_header, chunk) {
	// Ignore
};
